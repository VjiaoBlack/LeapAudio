...
window.CodeMirror = (function() {
  ...
  function buildToken(builder, text, style, startStyle, endStyle) {
    ...
    if (style || startStyle || endStyle || builder.measure) {
      ...
      if (fullStyle == 'cm-url-string') {
        return builder.pre.appendChild(elt("a", [content], fullStyle, undefined, content.data.substring(1,content.data.length-1), '_blank'));
      } else {
  ...
  function insertLineContent(line, builder, styles) {
    ...
    if (!spans) {
      for (var i = 1; i < styles.length; i+=2) {
        ...
        var styleClass = styleToClass(styles[i+1]);
        if (styleClass === 'cm-string' && styleName && /\.(json\.lz|js|css|html)["']$/.test(styleName)) {
          styleClass = 'cm-url-string';
        }
        builder.addToken(builder, styleName, styleClass);
      }
      return;
    }
  ...
  function elt(tag, content, className, style, href, target) {
